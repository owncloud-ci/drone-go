...
import (
	...
	"github.com/owncloud-ci/drone-go/plugin/logger"

	"github.com/99designs/httpsignatures-go"
)

// Handler returns a http.Handler that accepts JSON-encoded
// HTTP requests to convert the raw format to a yaml configuration
// file, invokes the underlying plugin, and writes the
// JSON-encoded config to the HTTP response.
//
...
func (p *handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	res, err := p.plugin.Convert(r.Context(), req)
	if err != nil {
		p.logger.Debugf("converter: cannot convert configuration: %s: %s: %s",
			req.Repo.Slug,
			req.Build.Target,
			err,
		)
